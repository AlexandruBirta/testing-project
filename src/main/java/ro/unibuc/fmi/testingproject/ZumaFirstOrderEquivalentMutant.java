...
@Slf4j
public class ZumaFirstOrderEquivalentMutant {
            ...
            for (int size = 0; size < currentSize; size++) {
                ...
                for (int boardIndex = 0; boardIndex < nowBoard.length(); boardIndex++) {
                    ...
                    for (char handCharacter : nowHand.toCharArray()) {
                        ...
                        // mutant is boardIndex >= 1 where previously condition was boardIndex > 1
                        if (1 == handCharactersFrequency.getOrDefault(handCharacter, 0) && boardIndex >= 1 && boardIndex < nowBoard.length() - 1 && nowBoard.charAt(boardIndex) != handCharacter && handCharacter != nowBoard.charAt(boardIndex + 1)) {
                            continue;
                        }
